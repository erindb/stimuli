...
var Stimuli = {
  ...
  Fish: function() {
    ...
    function draw(label, fangs, whiskers, scaleFactor) {
      ...
      var gradColor = Stimuli.makeGradient("0-", color);
      ...
      var getPathString = function(p) {
        ...
        var shortFatEnough = ErinTools.intermediate(sn, sb, bellyFatness);
        var tn = endpoints.tallNobelly[p];
        var tb = endpoints.tallBelly[p];
        var tallFatEnough = ErinTools.intermediate(tn, tb, bellyFatness);
        return ErinTools.intermediate(shortFatEnough, tallFatEnough, bodyTallness);
      }
      var paperCenter = [(Stimuli.containerWidth/2), (Stimuli.containerHeight/2)];
  ...
  drawPaths: function(paper, pieces, colors, interpolationFunction) {
    for (var i = 0; i < pieces.length; i++) {
      var piece = pieces[i];
